...
def category(name: str) -> str:
    ordered_prefixes = [
        ("attack_evasion_", "attack_evasion"),
        ("attack_ecc_", "ecc"),
        ("attack_", "attack"),
        ("margin_reachability_v2_native_", "tamper_softmin_v2_native"),
        ("margin_reachability_v2_", "tamper_softmin_v2_n1024"),
        ("mrc_ablation_", "tamper_softmin_v2_ablation"),
        ("single_tamper_", "tamper_single_copy"),
        ("payload_reachability_N1024_", "tamper_softmin_pilots"),
        ("softmin_diversity_", "tamper_softmin_pilots"),
        ("tail_margin_", "tamper_softmin_pilots"),
        ("contribution_softmin_", "tamper_softmin_ablation"),
...
def first_record(path: Path) -> dict[str, str]:
    with path.open(newline="") as f:
...
def publishable(path: Path) -> tuple[bool, int, list[str]]:
    if (path.name.endswith(".partial.csv") or ".shard_" in path.name
            or ".resume7_" in path.name
            or ".smoke." in path.name or ".timing." in path.name):
        return False, 0, []
    ...
    if path.name.startswith("attack_evasion_"):
        expected = 300 if first_record(path).get("model") == "timbrewm" else 600
        if rows != expected:
            return False, rows, columns
    if path.name.startswith("margin_reachability_v2_") and rows != 3000:
        return False, rows, columns
    if path.name.startswith("mrc_ablation_") and rows != 3000:
        return False, rows, columns
    if path.name.startswith("single_tamper_"):
        expected = 3000 if first_record(path).get("model") == "timbrewm" else 6000
        if rows != expected:
            return False, rows, columns
