...
SESSION=k8_population_native_20260830
OUT="$ROOT/data/k8_population_native_20260830"
LOG="$ROOT/results/logs/k8_population_native_20260830"
mkdir -p "$OUT" "$LOG"
...
for gpu in 0 1 2 3 4; do
  model=${models[$gpu]}
  cmd="cd '$ROOT' && export PYTHONPATH='/tmp/k5case_deps' && export WATERMARK_DEVICE='cuda:$gpu' && export WAVMARK_WINDOW_BATCH_SIZE=600 && '$PY' scripts/run_k8_population_native.py --model '$model' --output-dir '$OUT' > '$LOG/$model.log' 2>&1"
