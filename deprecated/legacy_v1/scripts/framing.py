...
def main():
    ...
    ap.add_argument("--trial_start", type=int, default=0,
                    help="Inclusive global trial index for an isolated shard.")
    ap.add_argument("--trial_end", type=int, default=None,
                    help="Exclusive global trial index; defaults to n_trials.")
    ap.add_argument("--output_tag", default="",
                    help="Optional filename tag for an isolated shard.")
    args = ap.parse_args()
    ...
    out_csv = RESULTS / f"{prefix}_{model}_K{K}{tag}.csv"
    partial_csv = RESULTS / f"{prefix}_{model}_K{K}{tag}.partial.csv"
    ...
    for gi, (spk, local_t) in enumerate(trial_idx):
        if gi < args.trial_start or gi >= trial_end:
            continue
        if gi in completed:
            continue
