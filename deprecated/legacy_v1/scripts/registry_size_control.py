...
registries are then deterministic prefixes of one per-trial random permutation,
so they are genuinely nested and always contain every coalition identity.
Restricting the already-computed score
...
def nested_registry_seed(spk: str, K: int, local_t: int) -> int:
    """One size-independent seed per trial for a genuinely nested sweep."""
    material = f"registry-control-nested|{spk}|K={K}|t={local_t}".encode()
    return int.from_bytes(hashlib.sha256(material).digest()[:8], "little")


def active_registries(full_size: int, coalition: list[int], sizes: list[int],
                      spk: str, K: int, local_t: int) -> dict[int, np.ndarray]:
    """Build R_N from prefixes of one permutation, so R_N is nested in N."""
    sizes = sorted(set(int(size) for size in sizes))
    if not sizes:
        return {}
    if sizes[0] < len(coalition) or sizes[-1] > full_size:
        raise ValueError(
            f"registry sizes {sizes} must lie in [{len(coalition)}, {full_size}]")
    coalition_arr = np.asarray(sorted(coalition), dtype=np.int64)
    pool = np.setdiff1d(np.arange(full_size, dtype=np.int64), coalition_arr,
                        assume_unique=False)
    rng = np.random.default_rng(nested_registry_seed(spk, K, local_t))
    order = rng.permutation(pool)
    registries = {}
    for size in sizes:
        if size == full_size:
            active = np.arange(full_size, dtype=np.int64)
        else:
            active = np.sort(np.concatenate([coalition_arr, order[:size - len(coalition_arr)]]))
        registries[size] = active
    for small, large in zip(sizes, sizes[1:]):
        if not np.isin(registries[small], registries[large], assume_unique=True).all():
            raise AssertionError(f"registry construction is not nested: N={small} -> N={large}")
    return registries


def active_registry(full_size: int, coalition: list[int], size: int,
                    spk: str, K: int, local_t: int) -> np.ndarray:
    """Single-size wrapper used by matched-N targeted experiments."""
    return active_registries(full_size, coalition, [size], spk, K, local_t)[size]
...
def main() -> None:
    ...
    parser.add_argument("--output_prefix", default="registry_control",
                        help="Filename prefix; use registry_control_nested for the corrected rerun.")
    parser.add_argument("--output_tag", default="",
                        help="Optional shard tag inserted before .csv.")
    ...
    for trial_id, (spk, local_t) in enumerate(trials):
        ...
        registries = active_registries(
            full_size, coalition, sizes, spk, args.K, local_t)
        ...
        if (trial_id + 1) % 10 == 0:
            ...
            print(f"{args.model} K={args.K}: trial={trial_id + 1}/{len(trials)} "
