...
BUNDLE_NAME = os.environ.get("AUDIT_BUNDLE_NAME", "paper_data_audit_20260820")
FREEZE_TIME = os.environ.get("AUDIT_FREEZE_TIME", "2026-08-20 20:04:17 CST")
...
def main() -> None:
    ...
    write_reports(manifest, source_notes, ev_trial, qp, pc, ca, t0, matched_detail, matched_any)
    ...
    archive=ROOT/f"{BUNDLE_NAME}.zip"
    if archive.exists(): archive.unlink()
    subprocess.run(["zip","-q","-r",str(archive),OUT.name],cwd=ROOT,check=True)


def write_reports(manifest, source_notes, ev_trial, qp, pc, ca, t0, matched_detail, matched_any):
...
Frozen audit time: {FREEZE_TIME}. No experiment was rerun by this audit.
...
`results/evaluation/` is the current runtime authoritative source: experiment scripts write final and checkpoint CSVs there. `data/` is a publication copy made by `scripts/publish_results_to_data.py`, not the runtime source. This bundle copied only complete final CSVs present at the frozen time; it did not consume `.partial.csv` as completed evidence. Any incomplete final file is isolated from complete-cell aggregation.
...
"""
    ...
    matched_df=pd.DataFrame(source_notes)
    matched_table=matched_df.to_markdown(index=False)
    wav_complete=(len(matched_df[(matched_df.file.str.contains("wavmark")) & matched_df.complete])==4)
    matched_rows=sum(x["rows"] for x in source_notes if x["complete"])
    matched_cells=sum(bool(x["complete"]) for x in source_notes)
    wav_status=("present and complete for K=2/3/5/8 (6,000 Mean+TCT target rows per K)"
                if wav_complete else "missing as a completed experiment; see the file audit below")
...
5. **WavMark matched-N arbitrary:** **{wav_status}**. No partial file is promoted.
...
- matched-N arbitrary: {matched_rows:,} complete rows across {matched_cells} model-K cells, each complete file containing Mean and TCT.
...
Currently unsupported: native arbitrary d_hull/margin claims for the four non-WavMark models; temporal SI_SDR; exact generating commit per individual historical CSV; attack-level coalition/hash, explicit seed and utterance ID because those fields were not stored.

No experiment was started by this audit. Remaining optional data-only gaps would require rerunning native arbitrary for AudioSeal/TimbreWM/VoiceMark/WMCodec while saving d_hull and target_margin, and temporal all models K=5 Mean/FWP at the desired n=300 while saving SI_SDR. These are not silently inferred from other outputs.
...
- Limitations: listed above; especially repeated-speaker dependence and absent native-arbitrary geometry for four models.
...
"""
    ...
    wav_detail=matched_detail[matched_detail.model=="wavmark"]
    wav_any=matched_any[matched_any.model=="wavmark"]
    wav_pt=binary_summary(wav_detail,["K","method"],"target_hit").to_markdown(index=False)
    wav_a10=binary_summary(wav_any,["K","method"],"any_of_10_hit").to_markdown(index=False)
    wav_files=matched_df[matched_df.file.str.contains("wavmark")].to_markdown(index=False)
    update=f"""# Post-run completion update

At {FREEZE_TIME}, the previously running WavMark matched-N arbitrary jobs had exited and all four final CSVs existed. GPU utilization was then zero and no matching `framing.py` process remained. This audit did not launch or alter those jobs.

## Final file completeness

{wav_files}

Each complete file has 6,000 data rows = 300 trials × 10 targets × 2 methods (`mean`, `tct`). The complete matched-N corpus now has {matched_rows:,} rows across {matched_cells} model-K cells.

## WavMark N=1024 per-target hit rates

{wav_pt}

## WavMark N=1024 any-of-ten hit rates

{wav_a10}

The two endpoints remain separate: per-target uses target rows as the denominator; any-of-ten uses one binary outcome per trial. Wilson 95% intervals are included. Complete all-model tables are `matchedN_arbitrary_per_target_by_model_K.csv` and `matchedN_arbitrary_any10_by_model_K.csv`.
"""
    (OUT/"04_postrun_completion_update.md").write_text(update)
