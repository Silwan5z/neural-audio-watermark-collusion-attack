"""盲方法：DM（Dispersion Maximization，用两两波形距离矩阵代替 median 残差 Gram）。

不重跑/不覆盖已有的 attack_*.csv（mean/fwp），只单独产出
dm_{model}_K{K}.csv，供后续与已有数据对齐比较（同一套 coalition_seed）。
...
- timbrewm K=5 n=60: DM ASR=0.967/R5esc=0.867/ACC=0.800，
  FWP ASR=0.967/R5esc=0.850/ACC=0.768 —— 基本打平，ACC 略逊
- timbrewm K=8 n=60: DM ASR=1.000/R5esc=0.967/ACC=0.790，
  FWP ASR=0.950/R5esc=0.800/ACC=0.800 —— DM 全面更优
- voicemark K=8 n=60: 四方法接近饱和，DM 并列最优
结论：K 越大 DM 相对 FWP 的优势越明显（K=2时数学上完全退化为
同一个解），K=5/8 上不差于 FWP，K=8 上明显更优。

用法：python scripts/blind_distance.py --model timbrewm --K 5 --n_trials 300
"""
...
def dm_weights(wavs, cap=0.5, n_random_starts=5):
    """max_a a^T D a，多起点避免局部解（含 FWP warm start）。"""
    D = dist_matrix(wavs)
    K = D.shape[0]
...
def main():
    ...
    out_csv = RESULTS / f"dm_{model}_K{K}.csv"
    rows = []
    t_start = time.time()
