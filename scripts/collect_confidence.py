...
3. Targeted: the highest-ranked exact Target-Bit Margin hit, when one exists.
...
def load_targeted_rows(root: Path, model: str) -> tuple[dict[int, dict], dict[int, dict]]:
    ...
    if len(source_rows) != 300:
        raise RuntimeError(
            f"expected 300 Target-Bit Margin trials for {model}, got {len(source_rows)}")
...
def main() -> None:
    ...
    for position, trial in enumerate(assigned, start=1):
        ...
        if targeted is not None and (trial, "targeted") not in complete:
            ...
            waveforms = [
                get_or_embed_native(
                    args.model, targeted["speaker"], payload,
                    int(targeted["clip_index"]) - 1)[0]
            ...
            probability, kind = bit_probabilities_native(
                args.model, attacked, sample_rate)
