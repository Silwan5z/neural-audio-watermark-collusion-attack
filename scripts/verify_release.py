#!/usr/bin/env python3
"""Read-only integrity checks for every result in the public release."""
...
ACTIVE_DATA = ("main", "k8", "coalitions", "targeted", "one_bit", "confidence", "summary")
...
def verify_confidence() -> None:
    ...
    print("PASS confidence data: all released condition counts match")


def verify_manifest() -> None:
    rows = read_csv(DATA / "MANIFEST.csv")
    listed = [row["path"] for row in rows]
    assert len(listed) == len(set(listed)), "duplicate paths in data/MANIFEST.csv"
    current = {
    ...
    assert set(listed) == current, (
        f"manifest mismatch: missing={sorted(current - set(listed))[:5]}, "
        f"stale={sorted(set(listed) - current)[:5]}")
    for row in rows:
        path = ROOT / row["path"]
        ...
        if path.suffix == ".csv":
            ...
            assert count == int(row["rows"]), path
...
def main() -> None:
    ...
    verify_confidence()
    verify_manifest()
    print("ALL RELEASE DATA CHECKS PASSED")
